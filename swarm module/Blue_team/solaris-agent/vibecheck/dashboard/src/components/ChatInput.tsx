...
import { Send, Loader2, Github, Shield } from "lucide-react";
import { isValidGitHubUrl, extractRepoName } from "@/lib/utils";

interface ChatInputProps {
  onSendMessage: (message: string, isRepoUrl?: boolean) => void;
  isLoading: boolean;
  placeholder?: string;
  disabled?: boolean;
}
...
export function ChatInput({
  ...
  useEffect(() => {
    setIsUrl(isValidGitHubUrl(input.trim()));
  }, [input]);
        ...
        {isUrl && (
          <div className="flex items-center gap-2 mb-2 px-3 py-2 rounded-lg bg-accent-primary/10 border border-accent-primary/30">
            ...
            <span className="text-sm text-accent-primary">
              Repository detected: {extractRepoName(input.trim())}
            </span>
          </div>
        )}
