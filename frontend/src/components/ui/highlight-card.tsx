...
const Component: FC<ComponentProps> = memo(({ title, description, icon }) => {
  return (
    <div className="group cursor-pointer transform transition-all duration-500 hover:scale-[1.02] hover:-rotate-1 h-full will-change-transform">
        ...
        <div className="p-8 relative z-10 flex flex-col items-center text-center h-full">
          ...
          <div className="space-y-1 max-w-sm flex-1">
            {description.map((line, idx) => (
              <p
                key={idx}
                className="text-gray-400 text-sm leading-relaxed transform group-hover:text-gray-200 transition-colors duration-300"
              >
                {line}
              </p>
            ))}
