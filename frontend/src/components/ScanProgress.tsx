...
function formatStageOutput(output: Record<string, unknown> | null): string[] {
    ...
    switch (stage) {
        ...
        case "detectors":
            ...
            break;
        case "semgrep":
            if (output.raw_findings !== undefined) {
                lines.push(`🐛 Semgrep found ${output.raw_findings} raw issues`);
                if (output.deduped_count) {
                    lines.push(`   • ${output.deduped_count} duplicates removed`);
                }
                if (output.skipped_test_fixtures) {
                    lines.push(`   • ${output.skipped_test_fixtures} test fixtures skipped`);
                }
                if (output.unique_files) {
                    lines.push(`   • ${output.unique_files} unique files affected`);
                }
                if (output.final_candidates !== undefined) {
                    lines.push(`✅ ${output.final_candidates} unique candidates for verification`);
                }
        ...
        case "llm_verification":
            ...
            if (output.progress !== undefined && output.total !== undefined) {
                const progress = Number(output.progress);
                const total = Number(output.total);
                const pct = output.percentage !== undefined ? Number(output.percentage) : Math.round((progress / total) * 100);
                lines.push(`🤖 LLM Verification: ${progress}/${total} (${pct}%)`);
                if (output.confirmed !== undefined || output.rejected !== undefined || output.errors !== undefined) {
                    const confirmed = Number(output.confirmed) || 0;
                    const rejected = Number(output.rejected) || 0;
                    const errors = Number(output.errors) || 0;
                    const saved = Number(output.saved_to_db) || 0;
                    lines.push(`   • ✅ ${confirmed} confirmed | ❌ ${rejected} rejected | 💾 ${saved} saved`);
                }
            } else if (output.total_candidates) {
            ...
            // Show latest verified finding in real-time
            if (output.latest_finding) {
                const finding = output.latest_finding as Record<string, unknown>;
                const status = finding.confirmed ? "✅" : "❌";
                const file = String(finding.file_path || "unknown").split("/").pop();
                const line = finding.line_start || 0;
                const vulnType = String(finding.vuln_type || "unknown").replace(/_/g, " ");
                lines.push(``);
                lines.push(`📄 Latest: ${status} ${vulnType} in ${file}:${line}`);
            }
            // Show recently verified list
            if (output.recently_verified && Array.isArray(output.recently_verified)) {
                const recent = output.recently_verified.slice(-5) as Array<Record<string, unknown>>;
                if (recent.length > 0) {
                    lines.push(``);
                    lines.push(`📋 Recent verifications:`);
                    recent.forEach((finding, idx) => {
                        const status = finding.confirmed ? "✅" : "❌";
                        const file = String(finding.file_path || "unknown").split("/").pop();
                        const line = finding.line_start || 0;
                        const vulnType = String(finding.vuln_type || "unknown").replace(/_/g, " ").substring(0, 20);
                        lines.push(`   ${status} ${vulnType} → ${file}:${line}`);
                    });
                }
            }
            break;
