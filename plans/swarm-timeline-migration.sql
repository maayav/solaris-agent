-- Migration: Swarm Timeline & Event Tracking Schema
...
BEGIN
    ...
    EXCEPTION WHEN duplicate_table THEN
        RAISE NOTICE 'swarm_events already in publication';
    END;
    
    -- Add swarm_exploit_attempts
    BEGIN
        ALTER PUBLICATION supabase_realtime ADD TABLE swarm_exploit_attempts;
    EXCEPTION WHEN duplicate_table THEN
        RAISE NOTICE 'swarm_exploit_attempts already in publication';
    END;
    ...
    EXCEPTION WHEN duplicate_table THEN
        RAISE NOTICE 'swarm_findings already in publication';
    END;
END $$;
...
-- ============================================================
-- 7. POPULATE FINDINGS FROM EXISTING EXPLOITS (if any)
-- ============================================================

-- This inserts findings for any successful exploits that don't have findings yet
...
SELECT 
  sea.mission_id,
  'Exploit: ' || sea.exploit_type as title,
  sea.evidence->>'description' as description,
  case 
    when sea.success then 'high'
    else 'info'
  end as severity,
  sea.exploit_type as finding_type,
  'gamma' as source,
  sea.target_url as target,
  sea.success as confirmed,
  'gamma' as agent_name,
  sea.evidence,
  sea.created_at
FROM swarm_exploit_attempts sea
WHERE sea.success = true
AND NOT EXISTS (
  SELECT 1 FROM swarm_findings sf 
  WHERE sf.mission_id = sea.mission_id
  AND sf.target = sea.target_url
  AND sf.finding_type = sea.exploit_type
)
