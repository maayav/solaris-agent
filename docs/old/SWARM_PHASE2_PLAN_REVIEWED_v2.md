...
**Version:** 2.0 (Incorporates Critical Review)
...
```python
def _compute_stall(state: RedTeamState, prev_state: RedTeamState) -> int:
    ...
    if current_hash == prev_hash:
        return state.get('stall_count', 0) + 1
    return 0  # New findings — reset stall

def _hash_findings(findings: list) -> str:
    """Stable hash of findings for comparison."""
...
> **CRITICAL:** Stall detection is computed deterministically in `commander_observe()` by comparing `previous_findings_hash` against the current findings hash. The LLM is NOT relied upon for stall detection — only for strategy decisions.

---

### Fix 3 — Authorization Enforcement Gate (CRITICAL)

**Root Cause:** HITL gate only fires mid-mission. No pre-flight authorization check. `needs_human_approval` flag can be bypassed. `verify_checksum()` is undefined — blocking this critical safety mechanism.
...
def verify_checksum(auth: 'AuthorizationContext') -> bool:
    """Verify HMAC signature matches computed checksum."""
...
async def preflight_authorization(state: RedTeamState) -> RedTeamState:
    ...
    if any(domain in target for domain in auth.excluded_domains):
...
> **NOTE:** `verify_vdp_scope()` fetches and parses the VDP policy page to verify the target is in scope. Implementation must handle Cloudflare-bypassed requests, 404s, and malformed policy pages.
...
> **TODO:** `file_tree` tool implementation not specified — must be added.
...
**Fix:** Correlated Success Criteria — **including XSS, SSRF, and path traversal.**
...
def deterministic_evaluate(result: ExploitResult) -> CriticVerdict:
    ...
    # XSS: reflected payload in response body (200 with echo)
    if vuln == 'xss' and status == 200:
        if result.injected_payload not in body:
            return CriticVerdict(success=False, reason='XSS: payload not reflected')
        # Check for actual script execution markers
        xss_markers = ['<script', 'javascript:', 'onerror=', 'onload=']
        if not any(marker in body for marker in xss_markers):
            return CriticVerdict(success=False, reason='XSS: payload echoed but no execution')
    ...
    if vuln == 'path_traversal' and status == 200:
        pt_indicators = ['root:', 'etc/passwd', 'C:\\Windows', '/etc/shadow']
...
> **NOTE:** Gamma must include `expected_output_fragment`, `baseline_response`, `injected_payload`, and `expected_ssrf_marker` in exploit result schema.

---

### Fix 7 — Stealth & Rate Control Strategy (MED)

**Root Cause:** No coordinated throttling. Also: the `yield` inside `async with` makes `acquire()` an async generator, not a context manager — callers would need `async for _ in throttle.acquire()` which is unnatural.

**Fix:** Unified Throttle Controller — **restructured as async context manager.**
...
class MissionThrottle:
    ...
    def _load_user_agents(self) -> list[str]:
        ...
        # ... load from wordlist or config

    async def __aenter__(self):
        """Async context manager entry — acquire throttle slot."""
        await self._semaphore.acquire()
        jitter = random.randint(0, self.config['jitter_ms']) / 1000
        await asyncio.sleep(jitter)
        if self.config.get('ua_rotate'):
            self._current_ua = random.choice(self._ua_list)
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        """Release throttle slot on exit."""
        self._semaphore.release()
        return False  # Don't suppress exceptions

    @property
    def current_ua(self) -> str | None:
        """Current user agent for stealth mode."""
        return getattr(self, '_current_ua', None)
```

**Usage (natural async with pattern):**
```python
async with MissionThrottle('stealth') as throttle:
    ua = throttle.current_ua  # Rotated UA available here
...
> **Note:** Original Sprint 1 was too aggressive. Splitting into 1a and 1b distributes load and ensures each piece is production-ready before moving on.
...
- [ ] Restructure `MissionThrottle` as async context manager
...
- [ ] Resolve Fix 2 (should_continue) — deterministic stall detection
...
> **CRITICAL FIX:** Replace `all-MiniLM-L6-v2` with `BAAI/bge-base-en-v1.5` or `jinaai/jina-embeddings-v2-base-en`. MiniLM conflates semantically similar security concepts (JWT kid SQLi vs JWT algorithm confusion).
...
CHUNK_STRATEGIES = {
    ...
    'hacktricks': chunk_by_h2_with_filter,  # Drops install/tool sections
...
**Token Propagation:**
```python
# Delta writes typed credentials
...
for cred in creds.credentials:
    if target_url in cred.scope:
        # Use this credential for the request
...
def _get_deterministic_throttle(mode: str, seed: int) -> MissionThrottle:
    ...
    random.seed(seed)
    throttle = MissionThrottle(mode)
    # Override jitter with seeded values
    return throttle
```
...
> **CRITICAL FIX:** Bandit alone is insufficient. Add semgrep with `python.lang.security` ruleset.
...
FORBIDDEN_PATTERNS = [
    ...
    r'requests\.post\(.*timeout\s*=\s*None',  # No timeout = potential DoS
]
```
...
-- RLS: Users can only see their own authorizations
CREATE POLICY "Users view own authorizations" ON swarm_authorizations
    FOR SELECT USING (authorized_by = current_user());

-- RLS: Users can only insert their own authorizations
CREATE POLICY "Users insert own authorizations" ON swarm_authorizations
    FOR INSERT WITH CHECK (authorized_by = current_user());
...
> **CRITICAL:** `redteam:cost:{mission_id}` is the **single source of truth** for budget enforcement. Supabase `swarm_cost_ledger` is async/audit only — never used for enforcement decisions. This prevents drift from worker crashes.
...
def evaluate_rag_precision(rag: HackingKnowledgeRAG, eval_queries: list) -> float:
...
| **Fix 3: Auth gate + checksum** | Live missions, MCP, Campaign | Sprint 0 (verify_checksum) |
| Fix 6: Critic hardening | Coach accuracy, mission learning | Sprint 0 (XSS/SSRF/PT) |
| Fix 7: Throttle fix | All tool execution | Sprint 0 (async context manager) |
| RAG store (2.1) | Per-agent doc injection; Coach; Playbooks | Sprint 0 (chunk_markdown) |
| Mission workspace (2.3) | Engineer tool output; Crawler; API storage | — |
| Auth agent (2.2) | Token chaining; Two-phase exploit precision | Sprint 0 (CredentialContext) |
| Crawler agent (3.1) | API discovery; OWASP classification | Sprint 0 (anti-detection) |
...
3. **Engineer agent has mandatory HITL gate + semgrep** — No auto-registration. Every tool requires human review + Bandit + semgrep with python.lang.security.
...
5. **Stall detection is deterministic** — Computed in graph layer by comparing findings hashes. LLM not trusted for safety-critical logic.
...
**Solution:** Opaque handle pattern with short-TTL Redis storage.
...
class CredentialVault:
    ...
    def store(self, mission_id: str, cred: CredentialContext) -> str:
        """Store credential, return opaque handle."""
        handle = secrets.token_urlsafe(16)
        key = f'{self.VAULT_PREFIX}{mission_id}:{handle}'
        
        # Encrypt before storing
        encrypted = self._encrypt(cred.model_dump_json())
        redis_client.setex(key, self.DEFAULT_TTL, encrypted)
        return handle
    
    def retrieve(self, mission_id: str, handle: str) -> CredentialContext | None:
        ...
        return CredentialContext.model_validate_json(decrypted)
...
|---|------|------------|--------|
| 1 | `verify_checksum()` | HMAC-SHA256 with server-side secret | Sprint 0 |
| 2 | `verify_vdp_scope()` | Fetch + parse VDP policy page | Sprint 0 |
| 3 | `chunk_markdown()` | Multi-strategy with H2, YAML, JSON, STIX | Sprint 0 |
| 4 | XSS/SSRF/PathTrav in Critic | Added to deterministic_evaluate() | Sprint 0 |
| 5 | Restructure MissionThrottle | Async context manager with __aenter__/__aexit__ | Sprint 0 |
...
| 10 | Mission replay seed | replay_seed field + seeded throttle | Sprint 4 |
