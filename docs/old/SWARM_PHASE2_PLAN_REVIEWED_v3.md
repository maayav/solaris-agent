...
> **CRITICAL:** `_compute_stall()` MUST be called at the start of `commander_observe()` before `should_continue()` reads `stall_count`. The `previous_findings_hash` update at the end of each iteration ensures the next iteration has the correct baseline for comparison.
...
def compute_authorization_checksum(auth: 'AuthorizationContext') -> str:
    ...
    NOTE: Use hmac.digest() or hmac.HMAC class — hmac.new() does NOT exist.
    """
    fields = [
        auth.type,
    ...
    # CORRECT: Use hmac.digest() — hmac.new() does not exist in Python's hmac module
    return hmac.digest(
...
- [ ] Implement `MissionWorkspace.write()` method
...
async def discover_api_endpoints(target: str, workspace: MissionWorkspace):
    ...
    # FIX: Use workspace.write() instead of undefined method
...
> **FIX:** Lambda closure bug — use default argument capture to bind tool name correctly.
...
class SwarmMCPBridge:
    async def register_mcp_server(self, server_url: str, server_name: str):
        ...
        for mcp_tool in tools:
            # FIX: Capture mcp_tool.name at lambda creation time, not at call time
            wrapped = ToolSpec(
...
CREATE TABLE swarm_authorizations (
    ...
    authorized_by   TEXT NOT NULL,  -- Stores auth.uid()::text or email
...
-- RLS: Users can only see/manage their own authorizations
-- FIX: Use auth.uid() or auth.email() instead of current_user()
...
**Solution:** Opaque handle pattern with Fernet encryption (handles key derivation, nonce, and MAC correctly).
...
class CredentialVault:
    ...
    def __init__(self, encryption_key: bytes | None = None):
        ...
        if encryption_key is None:
            encryption_key = os.environ['CREDENTIAL_VAULT_KEY'].encode()
        # Ensure key is valid Fernet key (URL-safe base64-encoded 32 bytes)
        self._fernet = Fernet(base64.urlsafe_b64encode(encryption_key[:32].ljust(32, b'\0')))
    ...
    def store(self, mission_id: str, cred: CredentialContext) -> str:
        ...
        # Encrypt credential JSON with Fernet (handles nonce + MAC)
        plaintext = cred.model_dump_json()
        ciphertext = self._fernet.encrypt(plaintext.encode())
        
        redis_client.setex(key, self.DEFAULT_TTL, ciphertext)
        return handle
    ...
    def retrieve(self, mission_id: str, handle: str) -> CredentialContext | None:
        ...
        """
        key = f'{self.VAULT_PREFIX}{mission_id}:{handle}'
        ciphertext = redis_client.get(key)
    ...
    def revoke(self, mission_id: str, handle: str) -> bool:
...
| 11 | `MissionWorkspace.write()` | Added with full implementation | Sprint 0 |
